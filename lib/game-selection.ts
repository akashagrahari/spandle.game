...
const DURATION_BUCKET_COUNT = 5;
const DECK_RECENCY_WINDOW = 4;
const DECK_RECENCY_PENALTY = 0.15;
...
function getSpacingBucket(durationYears: number): number {
  if (durationYears < 10) {
    return 0;
  }
  if (durationYears < 50) {
  ...
  if (durationYears < 200) {
    return 2;
  }
  if (durationYears < 1000) {
    return 3;
  }
  return 4;
}
...
function computeDurationBoundaries(durations: number[]): number[] {
  const sortedDurations = [...durations].sort((a, b) => a - b);
  const boundaries: number[] = [];

  for (let index = 1; index < DURATION_BUCKET_COUNT; index += 1) {
    const quantileIndex = Math.min(
      sortedDurations.length - 1,
      Math.floor((sortedDurations.length * index) / DURATION_BUCKET_COUNT),
    );
    boundaries.push(sortedDurations[quantileIndex]);
  }

  return boundaries;
}

function getDurationBucket(durationYears: number, boundaries: number[]): number {
  for (let index = 0; index < boundaries.length; index += 1) {
    if (durationYears < boundaries[index]) {
      return index;
    }
  }
...
function isCardUsed(
  card: PreparedCard,
  state: Pick<GameState, "usedQids" | "usedDurations">,
): boolean {
  return (
    state.usedQids.has(card.qid) || state.usedDurations.has(card.durationYears)
  );
}
  ...
  return played.every((playedCard) => {
    const playedCardSpacingBucket =
      "spacingBucket" in playedCard
        ...
        : getSpacingBucket(playedCard.durationYears);
    ...
    return (
      Math.abs(card.durationYears - playedCard.durationYears) >= minimumSpacing
    );
  });
}
...
function markCardUsed(state: GameState, card: PreparedCard): void {
  ...
  state.usedDurations.add(card.durationYears);
...
function chooseCardFromDeck(
  ...
  if (topPoolConfig) {
    ...
    const topRankedCandidates = [...deck.cards]
      ...
      .slice(0, topPoolSize)
      .filter((candidate) => {
        if (bucketFilter && !bucketFilter.has(candidate.durationBucket)) {
          return false;
        }
...
export function prepareDecks(
  ...
  const boundaries = computeDurationBoundaries(
    Array.from(cardsByDeckId.values()).flatMap((cards) =>
      cards.map((card) => card.durationYears),
    ),
  );
    ...
    .map(([deckId, cards]) => {
      ...
      return {
        cards: cards
          ...
          .map((card) => ({
            ...card,
            durationBucket: getDurationBucket(card.durationYears, boundaries),
            spacingBucket: getSpacingBucket(card.durationYears),
          })),
...
function buildOpeningBucketOrder(
  ...
  for (const deck of decks) {
    for (const card of deck.cards) {
      bucketCounts.set(
        card.durationBucket,
        (bucketCounts.get(card.durationBucket) ?? 0) + 1,
      );
    }
  }
  ...
  const firstBucket =
    weightedPick(
      availableBuckets,
      (bucket) => Math.sqrt(bucketCounts.get(bucket) ?? 0),
      random,
    ) ?? 0;
  const secondBucket =
    [...availableBuckets]
...
export function checkCorrect(
  ...
  const sorted = [...played, card].sort(
    (left, right) => left.durationYears - right.durationYears,
  );
