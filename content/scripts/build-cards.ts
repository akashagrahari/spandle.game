...
const MAX_REASONABLE_DURATION_YEARS = 10_000;
...
function parseOptionalYear(raw: string | undefined): number | null {
...
function buildCard(
  ...
  const subtitle = subtitleTemplate
    ...
    : "";
  const startYear = parseOptionalYear(row.startYear);
  const endYear = parseOptionalYear(row.endYear);
  ...
  const durationYears = effectiveEndYear - startYear;

  if (
    !Number.isFinite(durationYears) ||
    durationYears <= 0 ||
    durationYears > MAX_REASONABLE_DURATION_YEARS
  ) {
    return addReject(rejectionReasons, "unusable-duration");
  }
  ...
  return {
    ...
    endYear,
    fact: "",
    id: `${query.id}:${qid}:${startYear}:${endYear ?? "ongoing"}:${titleSlug || "card"}`,
    image: null,
    pageViews: null,
  ...
  const mergedCards = dedupeMergedCards(
    [...cards].sort(
      (left, right) =>
        ...
        left.durationYears - right.durationYears,
    ),
  );
