...
export default function CardVisual(props: Props) {
  ...
  const startLabel = formatYear(item.startYear);
  const endLabel = item.endYear === null ? "ongoing" : formatYear(item.endYear);
  const durationLabel = `${item.durationYears.toLocaleString()}y`;
  const pillLabel = `${startLabel}–${endLabel} · ${durationLabel}`;
